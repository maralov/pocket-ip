...
<?php if( get_row_layout() == 'trademark_search_block' ): ?>
    <div class="mb-80 content">
        ...
        <?php endif;?>
        <?php echo wpautop(get_sub_field('trademark_search_block_contnent',$term));  ?>
...
<?php if( get_row_layout() == 'search_block' ): 
    $title = get_sub_field('search_block_title');
    $description = get_sub_field('search_block_description');
?>
        <div class="mb-80 search-block cta-block cta-block-sm">

        <?php if($title):?>
            <p class="mb-2 h3"><?php echo  $title?></p>
        <?php endif;?>
        <?php if($description):?>
            <p class="paragrap mb-3"><?php echo $description?></p>
        <?php endif;?>
            <div class="d-flex align-items-end gap-2">
                <div class="w-100">
                    <label class="paragraph-sm text-white mb-lg-1" > <?php pll_e('Search parameter'); ?></label>
                    <div class="custom-select">
                        <button class="selected-values search-parameter" aria-expanded="false">
                            <span class="selected-values__text"><?php pll_e('Name'); ?></span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                                <path d="M7.5 10L12.5 15L17.5 10H7.5Z" fill="#999999"></path>
                            </svg>
                        </button>
                        <div class="select-dropdown js-select-search" style="display: none;">
                            <label class="search-option selected" data-search-param="name"><?php pll_e('Name'); ?></label>
                            <label class="search-option" data-search-param="applicationNumber"><?php pll_e('Application number'); ?></label>
                            <label class="search-option" data-search-param="registrationNumber"><?php pll_e('Registration number'); ?></label>
                            <label class="search-option" data-search-param="holderName"><?php pll_e('Holder name'); ?></label>
                            <label class="search-option" data-search-param="applicantName"><?php pll_e('Applicant name'); ?></label>
                        </div>
                    </div>
                </div>
                <div class="w-100">
                    <label class="paragraph-sm text-white mb-lg-1" for="search-value"> <?php pll_e('Search term'); ?></label>
                    <input type="text" id="search-value" class="input" placeholder="<?php pll_e('Enter search term...'); ?>" class="form-control me-2" required />
                </div>

                <a id="search-link" href="#" target="_blank" class="btn btn-primary"><?php pll_e('Search'); ?></a>
            </div>
        </div>
<?php endif; ?>
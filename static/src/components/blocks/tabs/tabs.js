document.addEventListener('DOMContentLoaded', function () {
	...
	function changeTab(event) {
		...
		tabPanels.forEach(function (panel) {
			if (
				panel.getAttribute('aria-labelledby') === selectedTab.getAttribute('aria-controls')
			) {
	...
	tabs.forEach(function (tab, index) {
		if (index === 0) {
			...
			var firstPanel = document.querySelector(
				'.tab-content[aria-labelledby="' + tab.getAttribute('aria-controls') + '"]'
			);
			if (firstPanel) {
		...
		tab.addEventListener('keydown', function (e) {
			...
			if (key === 37 || key === 38) {
				...
				if (tab.parentElement.previousElementSibling) {
					tab.parentElement.previousElementSibling
